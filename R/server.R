...
.module_palette <- function(levels) {
 ...
 output$network_plot <- renderVisNetwork({
    nd  <- network_data()      # dipende dal bottone Update
    lay <- input$network_layout # dipende dal menu -> forza la rivalutazione
    req(nd)

    ig_layout <- switch(lay,
      "fr"     = "layout_with_fr",
      "kk"     = "layout_with_kk",
      "drl"    = "layout_with_drl",
...
visNetwork(nd$nodes, nd$edges, height = "680px", width = "100%",
      ...
      visExport(type = "png", name = "tf_network", label = "Download PNG",
                float = "right",
