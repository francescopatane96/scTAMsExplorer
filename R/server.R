...
atlas_server <- function(seurat_obj, metadata_choices) {
  ...
  output$module_size_table <- renderDT({
    req(coexp_data())
  ...
  output$coexp_enrich_table <- renderDT({
    req(coexp_enrich_result())
    datatable(
      coexp_enrich_result() %>%
        select(Term, Overlap, P.value, Adjusted.P.value, Combined.Score, Genes) %>%
  ...
  output$reg_summary_table <- renderDT({
    req(reg_heatmap_data())
    datatable(
  ...
  output$reg_enrich_pos_table <- renderDT({
    req(reg_enrich_pos_df())
  ...
  output$reg_enrich_neg_table <- renderDT({
    req(reg_enrich_neg_df())
